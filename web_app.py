...
async def get_agent_tasks():
    ...
    # 检查各类待处理任务
    check_scripts = {
        "develop": "check-develop.sh",
        "fix": "check-fix.sh",
        "review": "check-review.sh",
        "integration": "check-integration.sh",
        "deploy": "check-deploy.sh",
        "ui": "check-ui.sh"
    }

    for task_type, script_name in check_scripts.items():
        ...
        if script_path.exists():
            try:
                ...
                # 解析输出，提取任务
                if output and "待" in output:
                    lines = output.split('\n')
                    for line in lines:
                        if line.startswith('#') or line.startswith('feature/'):
                            tasks[task_type].append(line.strip())
            except Exception as e:
                logger.warning(f"执行 {script_name} 失败: {e}")
    ...
    if tasks["develop"]:
        recommendations.append({
            "role": "develop2",
            "reason": f"有 {len(tasks['develop'])} 个待开发任务",
            "tasks": tasks["develop"][:3]
        })
    if tasks["fix"]:
        recommendations.append({
            "role": "bugfixer",
            "reason": f"有 {len(tasks['fix'])} 个待修复BUG",
            "tasks": tasks["fix"][:3]
        })
    if tasks["review"]:
        recommendations.append({
            "role": "test1",
            "reason": f"有 {len(tasks['review'])} 个待审查任务",
            "tasks": tasks["review"][:3]
        })
    if tasks["integration"]:
        recommendations.append({
            "role": "integration",
            "reason": f"有 {len(tasks['integration'])} 个待集成测试任务",
            "tasks": tasks["integration"][:3]
        })
    if tasks["deploy"]:
        recommendations.append({
            "role": "deploy",
            "reason": f"有 {len(tasks['deploy'])} 个待部署任务",
            "tasks": tasks["deploy"][:3]
        })
