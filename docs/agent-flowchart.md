...
flowchart TD
    ...
    subgraph DEV[develop1 / develop2]
        B1[bash .claude/scripts/dispatch.sh]
        B1 --> B2{>>> 待开发: N 个}
    ...
    subgraph TEST1[test1 代码审查]
        C1[bash .claude/scripts/dispatch.sh]
        C1 --> C2{>>> 待审查: N 个}
        C2 -->|N>0| C3[pytest + 审查]
        C3 --> C4{通过?}
    ...
    subgraph FIX[bugfixer 修复]
        D1[bash .claude/scripts/dispatch.sh]
        D1 --> D2{>>> 待修复: N 个}
        D2 -->|N>0| D3[修复代码]
    ...
    subgraph INT[integration 集成测试]
        E1[bash .claude/scripts/dispatch.sh]
        E1 --> E2{>>> 待集成测试: N 个}
        E2 -->|N>0| E3[端到端测试]
        E3 --> E4{通过?}
    ...
    subgraph UI[puppeteer UI验证]
        F1[bash .claude/scripts/dispatch.sh]
        F1 --> F2{>>> 待 UI 验证: N 个}
        F2 -->|N>0| F3[浏览器验证]
    ...
    subgraph DEPLOY[deploy 部署]
        G1[bash .claude/scripts/dispatch.sh]
        G1 --> G2{>>> 待部署: N 个}
...
> 兜底：各角色也可用独立 `bash .claude/scripts/check-*.sh` 脚本输出人类可读格式。
