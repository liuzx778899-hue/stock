...
develop → bash scripts/check-develop.sh 发现任务
  → checkout → 开发 → commit "feat(N): xxx, refs #N"
    → git tag -a round-N-dev -m '{JSON}'
      → test1 → bash scripts/check-review.sh 发现任务
        → 审查 → pytest → git tag -a round-N-review -m '{JSON}'
          → integration → bash scripts/check-integration.sh 发现任务
            → 集成测试 → git tag -a round-N-itest -m '{JSON}'
              → puppeteer → bash scripts/check-ui.sh 发现任务
                → UI验证 → git tag -a round-N-ui -m '{JSON}'
                  → deploy → bash scripts/check-deploy.sh 发现任务
...
Bug 链：
```
PM 建 bug Issue → bugfixer → bash scripts/check-fix.sh 发现任务
...
| 脚本 | 使用者 | 查找逻辑 |
|------|--------|------|
| `scripts/check-develop.sh` | develop1/2 | 未开始的 enhancement Issue（排除已在 pipeline 的） + 未打 dev tag 的分支 |
| `scripts/check-review.sh` | test1 | dev tag 无 review 的 round + fix-BUG 无 review 的 |
| `scripts/check-integration.sh` | integration | review tag 无 itest 的 |
| `scripts/check-fix.sh` | bugfixer | bug Issue 无 fix 分支的 + fix tag 待审查的 |
| `scripts/check-ui.sh` | puppeteer | itest tag 无 ui 的 |
| `scripts/check-deploy.sh` | deploy | ui tag 无 deploy 的 + master 有代码但 Issue 未关的（输出 `BRANCH=` 分支名） |
...
卡住时跑所有脚本检查断在哪一步：
```bash
bash scripts/check-develop.sh && bash scripts/check-review.sh && bash scripts/check-integration.sh && bash scripts/check-ui.sh && bash scripts/check-deploy.sh && bash scripts/check-fix.sh
```
